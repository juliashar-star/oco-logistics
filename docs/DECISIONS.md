...
2026-06-24 · Carrier picker v1: эвристики score (регион / способ / приоритет) не расширяем на грузовые P5/P6 до дописывания раздела в `docs/OCO_carrier_picker_spec_1.md` · почему: спека пока формализует только B2C; ad-hoc карты в `rank.ts` разойдутся с документом и сломают «спека = источник правды» · отвергли: добавлять score для ПЭК/ДЛ/… «на ходу» в коде (см. «Известный пробел v1: P5/P6» в спеке)
