...
- **2026-07-31 · postamat oversize — MARK in PVZ picker («не влезет»), do not hide, do not block.**
...
доставки» — котировка не отказывает, посылку могут снять с маршрута. Маркируем
как даркстор: продавец видит, какие локеры не берут текущую посылку, и сохраняет
выбор. Mark beats hide: посылка меняется, список точек грузится один раз, hide
убирал бы точки по причине, которую продавец не видит. Числа в одном месте
(`parcel-fits-pickup-point-kind`); 5Post 15/64/136 НЕ проверяем — нейтральный
`kind` не отличает 5Post от Market ПВЗ. Обычный ПВЗ сегодня без проверки.
Отвергли: прятать постаматы; блокировать заказ; длинную пометку в двухстрочном
select (адрес — единственное различие одноимённых точек).
