...
**ПОМЕТКА 06.09.2026, ЭТО НЕ ПРАВКА РЕШЕНИЯ — ЦИФРЫ ВЫШЕ ОТМЕНЕНЫ.** Ничего в абзаце не
изменено; решение, ради которого он написан, не затронуто. Но два числа в нём взяты со справочной
страницы, а 06.09.2026 прочитан первоисточник — «Условия оказания услуг корпоративным клиентам
сервиса Яндекс.Доставка», редакция 28.08.2026, раздел 3.2
(`contracts/yandex-usloviya-2026-08-28.md`; разбор — `docs/YANDEX.md` §2.1).

**Потолка 1 000 ₽ в договоре НЕТ.** Без объявленной ценности возмещение — **в размере двойной
стоимости услуги по организации доставки**. С объявленной ценностью потолок различается по видам
доставки: **250 000 ₽ отложенная, 500 000 ₽ экспресс**. Наблюдение о противоречии маркетинга и
справки остаётся верным; отменены только цифры. **Письменно уточнять у менеджера больше не нужно:
договор прочитан.**
...
## 2026-09-04 (4) · Ответ на вопрос перевозчику лежал в его же публичном документе, который мы не читали — и вопрос едва не ушёл письмом

**ЗАКАЗАНО ЮЛИЕЙ 04.09.** Код в этом срезе не менялся. Запись о том, чего стоило не прочитать
документ, который у перевозчика лежит открыто.

**ЧТО СЛУЧИЛОСЬ.** Вопрос **С-5** — «крайняя дата на карточке СДЭК означает прибытие в ПВЗ или
вручение покупателю?» — был сформулирован, обоснован, снабжён ссылками на маппер и на замер и лежал
в `CARRIER_QUESTIONS.md` со статусом «не отправлен». Он готов был уйти письмом менеджеру. **Ответ
всё это время стоял в п. 2.3.1 Регламента СДЭК** — публичного документа, который договор объявляет
своей неотъемлемой частью (п. 1.3) и на который сам же ссылается за размерами ответственности
(п. 5.3):
...
> Исполнителя/Постамат.

Гипотеза, записанная в С-5 как возможная, оказалась верной. Письмо не отправлялось и теперь не
понадобится.

**ЦЕНА ВОПРОСА — не одно письмо.** Вместе с ответом на С-5 из того же документа пришли: правило
исчисления сроков в РАБОЧИХ днях без дня приёма; правило пятницы; ответственность за просрочку
(3 % в день, но не более стоимости доставки); формула объёмного веса; право перевозчика пересчитать
средний вес при 3 % неверных габаритов; и подтверждение, что цена калькулятора не имеет силы оферты.
Шесть фактов, каждый из которых у нас либо отсутствовал, либо стоял как «не установлено», либо был
общим знанием без источника.

**ДВЕ ЗАПИСИ ОТМЕНЯЮТСЯ, И ОБЕ — ИЗ-ЗА ВТОРИЧНЫХ ИСТОЧНИКОВ.**

Первая: **«п. 8.2 Регламента — размер возмещения за утрату, 3000 ₽»**. Пункт назван неверно. 3 000
рублей — это **п. 8.1**, и он про отправления БЕЗ объявленной стоимости. П. 8.2 — про отправления С
объявленной стоимостью, и потолок там — **сама объявленная стоимость**. Мы унаследовали чужую
ошибку в номере из публичной цитаты юриста и носили её как факт с пометкой «вторичный источник».
Пометка честная, но она не спасла: цифра всё равно использовалась как рабочая.

**И она была неверна именно для нас.** Наш договор — тип ИМ, а для ИМ сбор за объявленную стоимость
**обязателен** (Приложение №1: 0,75 %), поэтому применяется 8.2, а не 8.1. То есть «3000 рублей»
не просто из другого пункта — оно из пункта, который к нашему договору не применяется вовсе.

Вторая: **«как соотносятся объёмный и физический вес — НЕ УСТАНОВЛЕНО»**. В `CDEK.md` стояло, что
договор правила выбора не называет, а формулировка «по большему из» снята как общее знание о рынке,
а не цитата. Снятие было правильным на тот момент и **теперь отменяется по существу**: п. 2.2.3
говорит «исходя из **наибольшего** значения», и даёт формулу — длина×ширина×высота/5000.

**ПОЧЕМУ ЭТО ПРОИЗОШЛО, И ЭТО НЕ ПРО ЛЕНЬ.** Регламент был **назван** — он стоял в `CDEK.md` как
единственная строка раздела «чего мы не читали», с объяснением, почему это важно. Знание о пробеле
было. Не было связи между пробелом и вопросами, которые он закрывает: С-5 не знал, что ответ может
лежать в Регламенте, а раздел «не читали» не знал, что от него зависит С-5. **Та же болезнь, что и
в записи `2026-09-04 (3)` про Ю1: факты есть, связи нет.** Второй раз за день.

**ПРАВИЛО, КОТОРОЕ ИЗ ЭТОГО СЛЕДУЕТ.** Прежде чем отправлять вопрос перевозчику — проверить, не
отвечает ли на него документ этого перевозчика, который у нас есть или который публично доступен.
Письмо стоит недели ожидания и часто остаётся без ответа: из пяти отправленных СДЭК и Яндексу
вопросов отвечен **один**. Чтение стоит часа.

**ЗАКРЫВ ОДНУ ДЫРУ, УВИДЕЛИ ВОСЕМЬ.** Раздел 11 Регламента перечисляет приложения поимённо: их
девять, у нас **одно**. Среди недостающих два, которые нужны раньше прочих: **Приложение №3
(упаковка и маркировка)** — обязательное условие услуги «Объявление стоимости», то есть той самой
страховки, вокруг которой у нас три боевых замера и два решения; и **Приложение №7 (соглашение об
обработке ПДн)** — рядом с ним лежат Ю2, Ю3, Ю4 и Ю12, которые задаются про 152-ФЗ, не зная, что
перевозчик уже написал о ролях сторон. **Это не ухудшение положения: восемь приложений
существовали всё это время, просто не были названы.** Названы теперь — `docs/CDEK.md` §5.

**ЧТО ЗАКРЫЛОСЬ НЕ ПОЛНОСТЬЮ, И ЭТО ВАЖНЕЕ ЗАКРЫТОГО.** Парный вопрос **Я-15** по Яндексу
**остаётся открытым**, потому что у Яндекса документа у нас нет вовсе. Подпись «быстрее» и приоритет
«самый быстрый» сравнивают поздний край срока СДЭК с поздним краем интервала Яндекса
(`offer-deadline.ts`). Теперь известно значение **одного края из двух**. Сравнение остаётся
недоказанным: половина ответа его не спасает, и записывать «сроки сравнимы» нельзя.

**ЗАВЕДЁН Ю13.** Правило Ю7 требует задавать вопрос про товарный знак заново по каждому
перевозчику; по СДЭК он **не задавался ни разу** — пробел вскрылся при чтении п. 2.5.9, который, в
отличие от оферты Яндекса, разрешение даёт. Но выдано оно **Клиенту**, а по модели F клиент —
продавец, не ОСО.

Отвергли: отправить С-5 письмом (ответ уже был написан); оставить «3000 ₽» рядом с прочитанным
пунктом как второе мнение (это не два мнения, а ошибка в номере); считать сравнимость сроков
закрытой, раз половина известна; чинить в этом срезе код, читающий `period_*` и `calendar_*` (ни
одно из полей не читается сегодня, и что они означают — по-прежнему не установлено).

## 2026-09-05 · Публичные документы перевозчика читаются ПЕРВЫМИ, а не последними; и два перевозчика выбрали разные конструкции обработки ПДн

**ЗАКАЗАНО ЮЛИЕЙ 05.09.** Код в этом срезе не менялся. Запись о методе и об одном факте, который
этот метод добыл.

### 1. МЕТОД: читать документы перевозчика до того, как платить за вопрос

**Правило.** Прежде чем задать вопрос юристу или отправить письмо перевозчику, **проверить, не
отвечает ли на него публичный документ этого перевозчика** — договор, регламент, приложения. Это
первый шаг, а не последний.

**Что метод дал за два дня, счётом:**

- **Закрыт вопрос перевозчику С-5** — что означает крайняя дата на карточке. Он был сформулирован,
  обоснован и готов уйти письмом; ответ стоял в п. 2.3.1 Регламента.
- **Отменены две наши записанные неправды.** Первая: «п. 8.2 Регламента — 3000 ₽». Пункт назван
  неверно — 3 000 ₽ это п. 8.1, про отправления БЕЗ объявленной стоимости, а к нашему договору
  типа ИМ он не применяется вовсе. Вторая: комментарий в `merge-tariff-services.ts` утверждал, что
  голая цена калькулятора «CORRECT, not understated», — Приложение №1 показало, что сбор
  обязателен и цена занижена на 0,75 % объявленной ценности.
- **Четыре вопроса из тринадцати переформулированы с «выясните факты» на «квалифицируйте
  известное»** — Ю2, Ю4, Ю10, Ю12. У каждого теперь раздел «что теперь факт из документа
  перевозчика» и раздел «что осталось юристу».
- **Восемь непрочитанных приложений превратились в два**, и оба непрочитанных не блокируют сегодня
  ни одного открытого вопроса.

**Почему это дешевле всего остального.** Вопрос юристу стоит денег и времени. Письмо перевозчику
стоит недели ожидания и часто остаётся без ответа: из пяти отправленных СДЭК и Яндексу вопросов
отвечен **один**. Чтение публичного документа стоит часа и не стоит ничего сверх.

**Почему это не было сделано раньше — и это не про лень.** Регламент был **назван**: он стоял в
`docs/CDEK.md` отдельной строкой раздела «чего мы не читали», с объяснением, почему это важно.
Знание о пробеле было. Не было **связи** между пробелом и вопросами, которые он закрывает. Та же
болезнь, что в записи `2026-09-04 (3)` про Ю1: факты есть, связи нет.

**Оборотная сторона метода, названная честно.** Чтение добавляет работы: закрыв Регламент, мы
увидели восемь приложений; прочитав приложения, получили порог 400 000 ₽, п. 8.5 про упаковку и два
новых продуктовых пробела. **Это не аргумент против метода.** Всё это существовало и до чтения —
просто было не названо, и продавец нёс риск, о котором ни он, ни мы не знали.

### 2. ФАКТ: два перевозчика выбрали РАЗНЫЕ конструкции обработки ПДн

**СДЭК — поручение обработки.** Приложение №7 к Регламенту определяет роли прямо: «Обработчик —
**Исполнитель**, которому… поручено обрабатывать ПДн» (п. 1.4); «Оператор — **Клиент**, который…
определяет цели обработки ПДн» (п. 1.5). Документ описывает **обе** конструкции сразу, и какая
применяется — решает цель: раздел 2 (самостоятельные операторы) отсылает к Приложению №1 к
Соглашению, где пять целей и **ни одна не про получателя** — только контрагентские данные о самих
сторонах. ПДн получателя стоят в Приложении №2, то есть **в поручении**.

**Яндекс — самостоятельные операторы.** По нашим записям раздел 10 оферты объявляет каждую сторону
самостоятельным оператором, а передачу — не поручением, и обязанность получить согласие возлагает
на передающую сторону.

**ЭТО ПОДТВЕРЖДАЕТ ПРАВИЛО Ю7 ФАКТОМ, А НЕ ОСТОРОЖНОСТЬЮ.** Ю7 требует задавать вопросы по оферте
заново по каждому перевозчику и не переносить ответ. До 05.09.2026 это было разумной
предусмотрительностью. Теперь у неё есть доказательство: **два перевозчика на одном и том же рынке
выбрали противоположные конструкции для одних и тех же данных одного и того же покупателя.** Ответ
по одному на другого не переносится не потому, что мы осторожны, а потому, что он неверен.

**Практическое следствие для ОСО, и оно открыто.** Ни в одном из документов **ОСО не упоминается**:
Приложение №7 описывает пару «продавец ↔ перевозчик», посредника в нём нет ни в одном пункте.
Значит место ОСО в цепочке приходится выводить, и оно может оказаться **разным для разных
перевозчиков**. Это записано как Ю2 и здесь не решается.

### 3. ОТМЕЧЕНО ПОПУТНО: подтверждение 152-ФЗ стоит на форме в ДВУХ местах

**Не задача этого среза, но факт записать надо, потому что он всплывёт, когда основание придётся
доказывать.**

Текст «Подтверждаю правовое основание обработки персональных данных получателя (152-ФЗ)» стоит в
`apps/web/components/new-order-form.tsx` **дважды — на строках 1597 и 2019**, дословно одинаковый.
**Мы не фиксируем, какое из двух нажато**, и не сохраняем ни текст согласия на момент нажатия, ни
его версию: в базе это булево поле.

**Почему это важно именно теперь.** Приложение №7, п. 2.5: передающая сторона по запросу
предоставляет документы, подтверждающие согласие либо иное правовое основание, **в течение 3
рабочих дней**. Оператор в этой конструкции — продавец. Значит однажды продавцу придётся доказать
основание, а предъявить он сможет только то, что сохранили мы. **Булево поле без текста и без
версии — слабое доказательство**, и два одинаковых чекбокса в одной форме делают его ещё слабее:
неизвестно даже, какой именно экран продавец видел.

Заодно отмечено: прежняя запись в Ю4 ссылалась на `new-order-form.tsx:1338` — **ссылка устарела**,
и это обнаружилось только при проверке. Номер строки в документе живёт ровно до следующей правки
файла.

Отвергли: считать вопрос закрытым, если документ отвечает на него частично (тогда «сужает», а не
«закрывает», и цитата обязательна); чинить чекбоксы в этом срезе; распознавать растровое
Приложение №4 через OCR (ошибку распознавания потом не отличить от опечатки перевозчика); называть
дату скачивания датой редакции там, где документ её не содержит.

## 2026-09-05 (2) · «Требует внимания» описано впервые: семь входов, одна надпись, и восемь строк, которым из неё не выйти

**ЗАКАЗАНО ЮЛИЕЙ 05.09.** Код не менялся. Запись о том, что показал разбор понятия, введённого
давно и не описанного нигде.

**ПОНЯТИЕ СУЩЕСТВОВАЛО ТОЛЬКО В КОДЕ.** `grep -rn "Требует внимания" docs/` до этого дня не давал
ни строки. Подпись задаётся в `apps/web/lib/shipments/labels.ts:15` и является прямым отображением
статуса `Shipment.status = PROBLEM`; комментариев к самому enum в `schema.prisma` нет ни одного.
Единственное описание смысла жило в комментарии над строкой перевода: «PROBLEM covers a lost parcel,
a carrier rejection, and our own submit failure». Теперь понятие описано — `docs/SHIPMENT_PROBLEM.md`.

**СЕМЬ ВХОДОВ, ДВА КЛАССА, ОДНА НАДПИСЬ.** От перевозчика: СДЭК `INVALID`; Яндекс
`VALIDATING_ERROR`; Экспресс `failed`, `estimating_failed`, `performer_not_found`; APIShip
`uploadingError`, `problem`, **`lost`**, **`unknown`**. Наш сбой при оформлении: ошибка авторизации
(`submit-order.ts:138`), любая неизвестная ошибка на confirm (`:144`), провал записи ПОСЛЕ успешного
confirm (`:199`), finally-нетто на застрявшем SUBMITTING (`:228`), любая ошибка легаси-пути
(`create-shipment.ts:227`).

**ПРИЧИНА НЕ СОХРАНЯЕТСЯ НИГДЕ.** Отдельного поля у PROBLEM нет; `returnReason` заполняется только
для RETURNED и CANCELED. Различие живёт в возвращаемом значении функции (`stage`, `reason`) и в
логах — **в строке `Shipment` от него не остаётся ничего.**

**И вот следствие, ради которого эта запись заказана: продавец видит красное и не может понять,
потерялась посылка или у нас упала сеть при оформлении.** Это разные его действия — в первом случае
звонить перевозчику и требовать возмещения, во втором проверить, не создался ли заказ дважды, и
оформить заново. Надпись одна, действия противоположные.

**ВЫХОД АВТОМАТИЧЕСКИЙ И МОЛЧАЛИВЫЙ.** PROBLEM не входит в `TERMINAL_STATUSES`
(`DELIVERED`, `RETURNED`, `CANCELED`), поэтому строка продолжает опрашиваться и перезаписывается
первым же маппируемым статусом **без единого условия** (`sync-yandex-statuses.ts:233`). Отправление
может само перестать требовать внимания, и следа в `Shipment` не останется — только строка в
`TrackingEvent`. Ручного выхода нет ни у продавца, ни у оператора.

**ЗАМЕР ПОКАЗАЛ ТУПИК, И ЭТО ГЛАВНОЕ ЧИСЛО ДНЯ.** В базе разработки 05.09.2026: CANCELED 19,
CREATED 14, **PROBLEM 8**, других статусов нет. Все восемь строк PROBLEM — **без `providerOrderId`,
без `trackNumber`, с нулём событий трекинга, без `providerKey`**, созданы с 03.07 по 02.09.

Из этого следует по построению: от перевозчика PROBLEM приходит только на строку, у которой есть чем
синхронизироваться. Здесь нет ничего. **Значит все восемь — из класса «наш сбой», и выйти им не по
чему:** синхронизация берёт их в кандидаты, но спрашивать у перевозчика нечего. **Эти строки будут
показывать красное вечно.**

**ЧТО ПРИ ЭТОМ СДЕЛАНО ОСОЗНАННО И ОСТАЁТСЯ ВЕРНЫМ.** «Не забрал получатель» у СДЭК идёт в RETURNED
(«it is the moment a return begins»), неудачная попытка вручения у Яндекса остаётся IN_TRANSIT
(«Yandex retries twice more»). Оба согласуются с правилом из `CLAUDE.md`: статус говорит, где
посылка, а не как мы к этому относимся; возврат — нормальный процесс. **Решение 13.07.2026 вести
«неизвестно» в PROBLEM, а не в DRAFT, тоже остаётся верным:** откат в DRAFT при реально созданном
заказе дал бы второго курьера.

**ОПОЗДАНИЕ НЕ ОТСЛЕЖИВАЕТСЯ ВООБЩЕ** — ни статуса, ни флага просрочки в схеме нет. Плановая дата у
нас есть, факта сравнения нет нигде. **И только теперь появилось, с чем сравнивать:** Регламент СДЭК
задаёт срок доставки, ответственность за его нарушение (3 % от стоимости доставки за каждый день,
Приложение №1, Таблица №2), смысл крайней даты (п. 2.3.1 — прибытие в ПВЗ) и правило пятницы. По
Яндексу это по-прежнему не установлено — Я-15 открыт.

**НАЙДЕНО ПОПУТНО: два источника одного понятия, один мёртвый.**
`apps/web/components/ui/status-badge.tsx` не используется нигде — поиск `StatusBadge` вне самого
файла пуст, — но несёт собственную карту подписей и цветов, включая ключ «требует внимания». Экран
отправлений держит свою карту с ДРУГИМИ классами. Опасность не в мёртвом коде, а в том, что он
выглядит живым: правка цвета в нём ничего не изменит на экране.

Отвергли: чинить что-либо из этого в том же срезе, где понятие только описано (сначала описание,
потом решение, иначе чиним то, чего не поняли); объединять «просрочку» с PROBLEM (разные вещи:
опоздание — это срок, PROBLEM — это событие); удалять мёртвый `status-badge.tsx` заодно (отдельная
правка кода, а этот срез — только документы).

## 2026-09-05 (3) · Оба перевозчика прочитаны в первоисточниках, и они выбрали противоположные конструкции — и по ПДн, и по товарным знакам

**ЗАКАЗАНО ЮЛИЕЙ 05.09.** Код не менялся. Запись о том, что дало чтение второго перевозчика, и о
методе, который сработал дважды.

### 1. ДВЕ ПРОТИВОПОЛОЖНОСТИ, ОБЕ ИЗ ТЕКСТА

**По обработке ПДн.** СДЭК — **поручение**: «Обработчик — Исполнитель, которому… поручено
обрабатывать ПДн» (Приложение №7, п. 1.4), «Оператор — Клиент» (п. 1.5), ПДн получателя стоят в
Приложении №2 к Соглашению, то есть в поручении. Яндекс — **самостоятельные операторы**, и
поручение исключено отрицанием: «Каждая из Сторон является оператором персональных данных»
(п. 10.1), «**Передача персональных данных не рассматривается Сторонами как поручение обработки
персональных данных**» (п. 10.2).

**По товарным знакам.** СДЭК — **разрешает**: «Исполнитель дает Клиенту согласие на использование
логотипа (товарного знака)» с семью ограничениями (Регламент, п. 2.5.9). Яндекс — **запрещает**:
«ни одна из Сторон не вправе каким-либо образом использовать полное, сокращенное фирменное
наименование, товарные знаки, коммерческие обозначения… принадлежащие второй Стороне, без
предварительного разрешения последней» (п. 12.10), с дверью в виде допсоглашения.

**ЭТО ПОДТВЕРЖДАЕТ ПРАВИЛО Ю7 ВТОРИЧНО И ПО ДРУГОЙ ОСИ.** 04.09 правило подтвердилось по ПДн, но
тогда одна половина была текстом, а вторая пересказом. Теперь **обе половины — текст**, и к ним
добавилась вторая ось: товарные знаки. Два перевозчика на одном рынке, одни и те же данные одного и
того же покупателя, одно и то же имя на одном и том же экране — **и противоположные конструкции по
обоим вопросам.** Ю7 больше не осторожность и не одно наблюдение: это закономерность, проверенная
дважды.

### 2. МЕТОД СРАБОТАЛ НА ВТОРОМ ПЕРЕВОЗЧИКЕ

Правило от 05.09 — читать публичный документ перевозчика ДО обращения к юристу — проверено повторно
и дало по Яндексу:

- **отменена неверная дата.** У нас стояло «оферта опубликована 21.05.2026»; документ называет
  **28.08.2026** («Дата публикации документа», «Дата вступления в силу»), а предыдущую редакцию —
  22.05.2026. Дата была неверна, и это выяснилось только чтением;
- **снято подозрение, которое выглядело серьёзным.** Пересказ п. 12.10 ссылался на «документ с
  офертой от 31.07.2026», редакции с такой датой в списке девяти версий нет. Проверка показала:
  **пункт существует под этим номером и с этим содержанием**. Основание маскировки имён устояло,
  ссылка на непонятный документ убрана из реестра;
- **четыре вопроса переформулированы** — Ю1, Ю2, Ю4, Ю8. Ю2 и Ю8 стояли на пересказе и теперь стоят
  на цитатах; Ю1 получил текст пункта вместо пересказа; Ю4 получил яндексовское требование
  «согласие **на передачу**», которого наш чекбокс не покрывает;
- **найден факт, которого в реестре не было** — асимметрия по логотипам (см. §3);
- **установлено отсутствие**, а это тоже знание: **срока доставки как обязательства в оферте нет
  вовсе**.

**Счёт по двум перевозчикам за два дня:** закрыт один вопрос перевозчику (С-5), отменены три
записанные неправды (п. 8.2 = 3000 ₽; «цена калькулятора не занижена»; дата оферты 21.05.2026),
переформулировано восемь вопросов юристу из тринадцати. **Ни один из этих результатов не стоил
ничего, кроме чтения.**

### 3. АСИММЕТРИЯ ПО ЛОГОТИПАМ — НОВЫЙ ФАКТ

Пока п. 12.10 запрещает Заказчику использовать знаки Яндекса, п. 3.2.6 даёт Яндексу
**«на условиях неисключительной лицензии безвозмездно использовать Логотип и иную информацию о
Заказчике на территории всего мира»**, любыми способами, **«в том числе в рекламе Яндекса»**, и
«не обязаны предоставлять какие-либо отчеты» об использовании. А п. 7.5 и 7.6 возлагают на
Заказчика полную ответственность за содержание и форму этого логотипа, включая возмещение убытков
Яндексу по претензиям третьих лиц.

**Вопрос, который отсюда вырос и которого раньше не было:** понимает ли продавец, передавая логотип
через наш экран, объём выданной лицензии, и обязаны ли мы ему это сообщать. Записан в Ю1.

### 4. ЧТО ЭТО МЕНЯЕТ ПРЯМО СЕЙЧАС

**Претензия за просрочку возможна ровно по одному перевозчику из двух.** У СДЭК срок доставки —
обязательство, нарушение стоит 3 % от стоимости доставки за каждый день (Приложение №1, Таблица
№2). У Яндекса обязательства по сроку нет вовсе, значит и нарушать нечего. Строка в
`docs/ROADMAP.md` исправлена: это **установленный факт**, а не «не установлено».

**И структурная вещь, общая для обоих: ОСО не сторона ни одного из этих договоров.** Ни Приложение
№7 СДЭК, ни раздел 10 оферты Яндекса посредника не упоминают. Место ОСО в цепочке приходится
выводить, и оно **может оказаться разным для разных перевозчиков** — это и есть то, что осталось
юристу в Ю2.

### 5. ЗАКРЫТАЯ ДЫРА ПОКАЗАЛА СЛЕДУЮЩУЮ — ВТОРОЙ РАЗ ЗА ДВА ДНЯ

Оферта Яндекса девятнадцать раз отсылает к **«Условиям оказания услуг корпоративным клиентам»**
(https://yandex.ru/legal/logistics_terms_of_use/), которых у нас нет. Туда уходит именно то, чего в
оферте не хватает: сами услуги (п. 1.2), порядок возмещения (п. 4.1.5.2 → «п. 3.2 Условий»),
особенности отдельных тарифов (п. 1.20). **По роли это аналог Регламента СДЭК.**

04.09 прочитанный Регламент открыл восемь приложений. 05.09 прочитанная оферта открыла «Условия».
**Это норма работы с чужими документами, а не неудача:** документ, к которому отсылают, существовал
и раньше — просто не был назван. Записано в `docs/YANDEX.md` §5.

Отвергли: считать, что «пересказ подтвердился» = «вопрос закрыт» (подтвердилось содержание, а
квалификация осталась юристу); удалять замещённые формулировки после переписывания (они показывают,
из какого прочтения вырос вопрос, и одна из них оказалась верной ровно наполовину); чинить дату
21.05 правкой на день — откуда она взялась, неизвестно, поэтому запись отменена, а не поправлена;
трогать записи про «документ от 31.07.2026» там, где он означает **замер API**, а не текст оферты —
это два разных смысла одной даты, и путать их нельзя.

## 2026-09-05 (4) · Вывод, сделанный на одном тарифе, продержался неделю — и правило, которое из этого следует

**ЗАКАЗАНО ЮЛИЕЙ 05.09.** Код не менялся. Запись о том, как одно наблюдение стало общим
утверждением, неделю читалось как факт и попало в карту знания.

### ЧТО СЛУЧИЛОСЬ

**28.08.2026** третий боевой прогон проверил потарифный метод `/v2/calculator/tariff` на **одном**
коде — `61`, механически взятой первой строке выдачи. Получил два результата: метод отвечает 200, а
с `INSURANCE` отвечает 400. И записал два вывода:

> **обязательная страховка на этом договоре не котируется НИ ОДНИМ методом, до которого мы можем
> дотянуться** … то есть **отказ принадлежит ДОГОВОРУ, а не методу.**

**05.09.2026** четвёртый прогон проверил ещё четыре кода. Три новосибирских ответили так же — 400.
**Московский `2360` ответил 200 и посчитал сбор:** `sum: 50.00` при параметре `"5000"`,
`vat_rate: 22.00`, `total_sum: 61.00`. Без параметра тот же тариф подставляет сбор сам — `sum:
30.00`.

**Отказ принадлежит ТАРИФУ.** Один договор, один метод, один прогон, одиннадцать вызовов — и внутри
него три отказа и один расчёт.

### ПОЧЕМУ ЭТО ПРОДЕРЖАЛОСЬ НЕДЕЛЮ

**Не потому, что кто-то соврал или поленился.** Файл 28.08 сам, в §5, честно записал:

> **НЕ ИЗМЕРЕНО: ведут ли себя остальные 26 кодов так же.** Проверен один — `61`, первая строка.
> Вывод «метод открыт» опирается на один успешный код, и этого достаточно, чтобы закрыть вопрос
> «закрыт ли метод целиком»; вывод «`INSURANCE` недоступен» опирается на тот же один код.

**Оговорка была. Она просто лежала не там.** Вывод стоял в §4 как результат, а ограничение — в §5
как «не измерено». Дальше в карту `docs/CDEK.md` переехал **вывод**, без ограничения. С этого
момента утверждение читалось как установленный факт, а строка, объяснявшая его хрупкость, осталась
в исследовательском файле, куда никто не заглядывает, приняв решение.

**Та же болезнь, что в записях `2026-09-04 (3)` и `2026-09-05`: факт есть, связь потеряна.** Третий
раз за два дня, и каждый раз в новой форме — сначала решение без основания, потом вопрос без
документа, теперь вывод без своей оговорки.

### ЧЕГО ЭТО СТОИЛО

Утверждение «сбор не котируется нигде» неделю определяло картину: считалось, что цена на карточке
СДЭК занижена **всегда** и сделать с этим ничего нельзя. Правильная картина — **занижение зависит
от тарифа**: там, где сбор приходит или принимается, цена полная. Строка в `docs/ROADMAP.md`
переписана: задача не отменяется, но меняет объём — надо знать, **на каких** тарифах занижение
есть, а это по-прежнему не установлено.

Вместе с этим опровергнута и третья запись, из ADR от 24.08: «эта цифра не может зависеть от
стоимости товара продавца». **Может:** на `2360` объявленная стоимость передаётся и сбор от неё
меняется, 30 → 50.

### ПРАВИЛО, КОТОРОЕ ИЗ ЭТОГО СЛЕДУЕТ

**Утверждение вида «ни одним методом», «на нашем договоре», «нигде», «всегда» требует БОЛЕЕ ОДНОГО
наблюдения. Одно наблюдение доказывает только себя.**

Практически это значит три вещи.

1. **Один код, один тариф, один маршрут — это факт об этом коде, тарифе и маршруте.** Чтобы
   написать «на договоре», нужно показать, что при смене тарифа результат не меняется. Мы этого не
   показывали и всё равно написали.
2. **Оговорка «не измерено» живёт вместе с выводом, а не в другом разделе.** Если вывод переезжает в
   карту знания, ограничение переезжает с ним — иначе в карте оказывается утверждение шире
   доказательства.
3. **Отрицательный результат особенно опасен.** «Не работает нигде» звучит как окончательный ответ и
   закрывает поиск; «не работает здесь» оставляет вопрос открытым. Первое надо доказывать строже
   второго, а не мягче.

**Стоимость соблюдения — одиннадцать вызовов к калькулятору.** Стоимость несоблюдения — неделя с
неверной картиной и три записи, которые пришлось отменять.

### ЧТО ПРИ ЭТОМ НЕ МЕНЯЕТСЯ

**Правило `insurance-services.ts` для типа 1 — не отправлять `services` — остаётся.** Оно стоит не
на опровергнутом выводе, а на другом замере: отправка `services` роняет **весь список**, 37 строк из
38 (14.08). Это по-прежнему так, и замер 05.09 его не касался. Опровергнуто «сбор не котируется
нигде», а не «отправлять `services` опасно».

**И два числа остались несведёнными, оба измеренные.** Ставка: боевой замер даёт ровно **1 %**
(30/3000 и 50/5000), Приложение №1 называет **0,75 %** для типа ИМ. `vat_rate`: у нас **22**,
измерено трижды; **5** записано у интегратора на чужом договоре и не объяснено с 24.08. Ни одно из
двух этот прогон не свёл, и здесь они не сводятся.

Отвергли: тихо поправить формулировки в карте (отменённое помечается датой и ссылкой на замер, иначе
через месяц не понять, почему написано именно так); начать отправлять `services` на основании
одного успешного тарифа (та же ошибка обобщения, только в обратную сторону); объяснять расхождение
1 % и 0,75 % догадкой о том, что у тарифа своя ставка, — это правдоподобно и не измерено.

## 2026-09-06 · Третий раз за трое суток вторичный источник оказался неполон — и куда именно смещалась ошибка

**ЗАКАЗАНО ЮЛИЕЙ 06.09.** Код не менялся. Запись о повторяющейся ошибке и о том, что в ней
закономерно, а что нет.

### ТРИ СЛУЧАЯ, ПОДРЯД

**Первый, 04.09.2026 — «п. 8.2 Регламента СДЭК: 3 000 ₽».** Источник — публичная цитата юриста,
вторичный. Прочитанный Регламент показал: **3 000 ₽ это п. 8.1**, про отправления БЕЗ объявленной
стоимости, а к нашему договору типа ИМ он не применяется вовсе — у нас сбор обязателен, значит
работает п. 8.2, где потолок **объявленная стоимость плюс стоимость доставки**.

**Второй, 06.09.2026 — «Яндекс: не более 250 000 ₽».** Источник — справочная страница
`yandex.ru/support/delivery-profile/…/insurance-cargo-other-day`, вторичный. Прочитанные Условия
показали: потолок **различается по видам доставки** — 250 000 ₽ отложенная, **500 000 ₽ экспресс**.

**Третий, там же — «Яндекс: 250 000 ₽ в «Экспресс» и НДД, 500 000 ₽ в «Грузовом»».** Источник —
страница `yandex.ru/legal/insurance`, вторичный. **Неверно по обеим половинам:** экспрессу приписан
вдвое меньший потолок, а «Грузовой» договором в этом контексте не упоминается.

### КУДА СМЕЩАЛАСЬ ОШИБКА — И ЗДЕСЬ НАДО БЫТЬ ТОЧНЫМ

**Во всех трёх случаях смещение одинаковое: в сторону ЗАНИЖЕНИЯ прав продавца.** 3 000 ₽ вместо
объявленной стоимости; 250 000 вместо 500 000; 250 000 вместо 500 000 снова. Ни разу вторичный
источник не преувеличил того, что причитается.

**НО ЕСТЬ ЧЕТВЁРТАЯ ЗАПИСЬ ИЗ ТОГО ЖЕ БЛОКА, И ОНА ЛОМАЕТ ЧИСТОТУ ЗАКОНОМЕРНОСТИ.** «Без оценочной
стоимости: 1 000 ₽ для «По городу» и «По России», 3 000 ₽ для «Экспресс», 10 000 ₽ для «Грузового»»
— договор вместо фиксированных сумм даёт **двойную стоимость услуги по организации доставки**. Это
**не занижение и не завышение, а другая механика**, и знак зависит от цены доставки:

```
доставка   300 ₽ → двойная    600 ₽ — МЕНЬШЕ записанной 1 000 ₽
доставка   500 ₽ → двойная  1 000 ₽ — вровень
доставка 2 000 ₽ → двойная  4 000 ₽ — БОЛЬШЕ записанных 1 000 и 3 000 ₽
```

**То есть «все три раза одинаково» верно для трёх случаев про потолки и неверно для четвёртого.**
Записываю так, а не красивее: закономерность есть, но она не универсальна, и подгонять под неё
четвёртый случай значило бы повторить ту же ошибку — принять правдоподобное за проверенное.

### ПОЧЕМУ СМЕЩЕНИЕ ИМЕННО ТАКОЕ — ГИПОТЕЗА, НЕ ФАКТ

Справочные и маркетинговые страницы пишутся для массового читателя и называют **один** случай —
самый частый или самый простой. Договор описывает **все** случаи, включая редкие и дорогие. Поэтому
пересказ справки систематически теряет верхние ветки: они реже и объяснять их дольше. **Это
объяснение правдоподобно и не проверено.** Что проверено — сам факт смещения в трёх случаях из
четырёх.

### ПРАВИЛО

**Вторичный источник о деньгах не может быть основанием ни для обещания продавцу, ни для записи в
карту знания. Он годится только как повод прочитать первичный.**

Практически это три вещи.

1. **Число, взятое со справочной или маркетинговой страницы, помечается источником прямо в той же
   строке** — и помечается **вторичным**, а не просто «источник такой-то». Это правило у нас уже
   было и соблюдалось: файл 09.07 честно писал «официальные источники Яндекса противоречат друг
   другу» и перечислял четыре страницы с четырьмя цифрами. **Пометка была. Дальше по репозиторию
   уехали числа, а предупреждение осталось на месте** — та же болезнь, что в записи `2026-09-05 (4)`
   про вывод, сделанный на одном тарифе.
2. **Если чисел несколько и они расходятся, в карту не переезжает ни одно** — переезжает факт
   расхождения и вопрос. Одно из четырёх выбирать нельзя, даже если оно выглядит самым надёжным.
3. **Обещание продавцу о деньгах строится только на первоисточнике.** Ни одна из трёх ошибок не
   дошла до экрана, и это удача, а не заслуга: подпись «страховая сумма» на форме нового заказа
   стоит там до сих пор и опирается на такую же цепочку.

### ЧТО ИСПРАВЛЕНО ЭТИМ СРЕЗОМ

Отменены, но **не удалены**, шесть записей в пяти файлах: блок о пределах возмещения Яндекса в
`docs/research/pd-quoting-declared-value-2026-07-09.md` (три записи одним помеченным блоком), цифры
в `docs/research/yandex-delivery-api-2026-07-10.md`, датированная пометка в `docs/DECISIONS.md` к
записи от 09.07 — файл append-only, поэтому именно пометка, а не правка.

**Отдельно разведены два числа 250 000, которые означают РАЗНОЕ и совпали значением:** предел
возмещения (Условия, раздел 3.2) и потолок объявленной ЦЕННОСТИ, принимаемой API (вопрос Я-2).
Договор второго не устанавливает нигде, поэтому Я-2 остаётся открытым. Разводящие пометки — в
`docs/CARRIER_QUESTIONS.md`, `docs/YANDEX.md` и `docs/LIVE_ENVIRONMENT_TASKS.md`. **Это тот же
случай, что «документ от 31.07» вчера: одна строка, два смысла, и стереть надо было только один.**

Отвергли: удалить неверные записи вместо пометки (они показывают, что говорили публичные страницы
Яндекса на 09.07.2026, и это отдельный факт); подогнать четвёртый случай под закономерность трёх;
объяснить смещение умыслом перевозчика — правдоподобно, недоказуемо и не нужно для вывода.

## 2026-09-08 · Ограничение частоты: почему смена пароля считает по аккаунту, а подтверждение адреса считает всё подряд

**ЗАКАЗАНО ЮЛИЕЙ 08.09.** Код этой записью не меняется: оба решения приняты 07.09.2026 коммитом
`c2378a0`, но запись после себя тот срез не оставил — вот она, днём позже. В коде видно, ЧТО
выбрано; здесь записано ПОЧЕМУ, потому что из кода это не выводится.

### РЕШЕНИЕ 1. Ключ ограничения на смене пароля — ТОЛЬКО `userId`

`apps/web/app/api/user/password/route.ts` считает попытки по одному признаку — идентификатору
пользователя. Ни адрес, ни пара «адрес + аккаунт».

**Почему не пара.** Составной ключ выдаёт КАЖДОЙ комбинации собственный свежий лимит, и обходится
ротацией любой из двух половин: сменил адрес — счёт начался заново. Тому, кто уже держит чужую
сессию, ротация адресов не стоит ничего. Честному человеку та же конструкция вредит зеркально:
перешёл с домашней сети на мобильную — и его собственный счётчик остался на прежнем ключе. То
есть пара одновременно слаба против того, против кого защищает, и груба к тому, кого защищает.

**Почему ключ по аккаунту безопасен ЗДЕСЬ, хотя на логине был бы опасен.** Обычное возражение
против ключа-аккаунта — «чужой аккаунт можно запереть, набивая неверные пароли». На этом роуте
запирать некого: до него не дойти, не владея сессией этого самого пользователя — он стоит за
`withAuth`. Запереть можно только себя. На логине возражение остаётся в полной силе, поэтому
решение туда НЕ переносится по аналогии.

⚠ **РОУТ ЛОГИНА СЕГОДНЯ ИСПОЛЬЗУЕТ ИМЕННО ПАРУ И ЗНАЧИТ ПОДВЕРЖЕН ТОМУ САМОМУ ОБХОДУ.**
`apps/web/app/api/auth/login/route.ts`, функция `clientKey` — ключ собирается как адрес, двоеточие
и почта, и в таком виде уходит в `isLoginBlocked`. Подбор пароля к одному адресу растягивается на
сколько угодно попыток простой сменой IP. **Этим срезом не исправлено, остаётся открытым.** Как
чинить — здесь не решено: перенос ключа на аккаунт воскрешает ровно то возражение о запирании
чужого входа, от которого мы только что отговорились на смене пароля, так что это отдельное
решение, а не перенос готового.

### РЕШЕНИЕ 2. Что считается попыткой — и почему на двух роутах по-разному

**`password-change` считает только НЕУДАЧИ, а успех счётчик сбрасывает.** Это модель логина и
регистрации, а не пяти роутов, которые пишут каждый запрос. Основание простое: сменить пароль
несколько раз подряд — не злоупотребление, а человек, который ошибся и затем ввёл верный текущий
пароль, тем самым доказал, что он владелец, и нести ошибки дальше ему незачем.

**`verify-email` считает КАЖДЫЙ запрос и сброса не имеет.** Не по недосмотру и не ради симметрии:
«считать только неудачи» на этом роуте НЕВЫРАЗИМО. Он отвечает одними редиректами, и успех от
промаха отличается ТОЛЬКО тем, на какую страницу попал браузер; отделить неудачу, чтобы посчитать
именно её, нечем. А не считать ничего — значит оставить перебор токена неограниченным.

**СЛЕДСТВИЕ, И ЗАПИСЫВАЕМ ЕГО ЧЕСТНО.** Строка в `RateLimitBucket` пишется на КАЖДЫЙ переход по
ссылке из письма — в том числе на запросы вообще без токена и на переходы, которые делает не
человек, а сканер ссылок или панель предпросмотра почтового клиента. Чистки просроченных строк
нет никакой — ни фоновой, ни при чтении (принято при P0-SEC4 как компромисс MVP-масштаба: индекс
по `resetAt` в схеме есть, самой уборки нет). Значит таблица растёт теперь не только от перебора,
но и от совершенно нормальной работы. Это цена, заплаченная сознательно, а не просмотренная.

### ЧЕГО В ЭТОЙ ЗАПИСИ НЕТ

У механизма ограничения частоты по-прежнему нет ни одного теста — ни у двух новых бакетов, ни у
семи прежних, — и написать его сегодня нельзя: модуль берёт `prisma` из синглтона и не принимает
клиента параметром. Это не решение, а открытый хвост, и живёт он в `docs/ROADMAP.md`; здесь
только решения.

Отвергли: пару «адрес + аккаунт» на смене пароля (обходится ротацией половины и наказывает
честного при смене сети); счёт одних неудач на `verify-email` (отделить их нечем); заодно
перенести решение 1 на роут логина (там возражение о запирании чужого аккаунта живо — отдельное
решение, а не следствие этого).

## 2026-09-08 (2) · Обязательное поле наконец помечено, и дашборд перестал поздравлять недоделавшего

**ЗАКАЗАНО ЮЛИЕЙ 08.09.** Срез E1: два дефекта кабинета, оба разметочные, в разных файлах, ничего
не вычисляющие.

### РЕШЕНИЕ 1. Обязательное поле помечается КРАСНОЙ ЗВЁЗДОЧКОЙ после подписи — через единственный компонент

`apps/web/components/ui/field-label.tsx`, компонент `FieldLabel`. Он несёт подпись, значок и связь
с контролом; вызывающий передаёт текст, признак обязательности и `htmlFor`.

**Принятого способа не было ВООБЩЕ, и это измерено, а не предположено.** По всему `apps/web`: ни
одного `aria-required`, ни одной звёздочки в подписи, а слово «обязательно» встречалось
единственным образом — плейсхолдером «Необязательно» на форме подбора перевозчика, то есть в
противоположном смысле. Восемь контролов формы нового заказа несли голый атрибут `required` и
выглядели ровно как необязательные соседи: первым, что сообщало продавцу о правиле, был отказ
браузера отправить форму.

**ОДИН КОМПОНЕНТ, А НЕ ВОСЕМЬ КОПИЙ РАЗМЕТКИ.** Ещё семь файлов кабинета содержат `required`;
значок, вставленный в каждый руками, — это восемь решений, которые расходятся, и расхождение
невидимо до того дня, когда два экрана заговорят по-разному перед продавцом.

**ЗВЁЗДОЧКА — ДЛЯ ГЛАЗ, ПРАВИЛО — ДЛЯ ПРОГРАММ.** Значок помечен `aria-hidden`: читалка произнесла
бы «звёздочка», а это название значка, а не правила. Обязательность идёт отдельно — атрибутом
`aria-required` на самом контроле, и ставит его вызывающий, потому что подпись не может нести
состояние чужого элемента.

**`htmlFor` — ОБЯЗАТЕЛЬНЫЙ АРГУМЕНТ, а не необязательный.** Все восемь заменённых подписей были
висячими `<label>` без `htmlFor`, а контролы — без `id`: текст, который просто стоял сверху, и
клик по нему не делал ничего. Сделав связь обязательной в сигнатуре, мы лишаем следующую форму
возможности повторить это по забывчивости.

**Цвет — токен палитры `text-error`** (`--error`, `#dc2626`), единственный красный, который проект
объявляет: `apps/web/app/globals.css` и `apps/web/tailwind.config.ts`.

**Семь остальных файлов с `required` переезжают ПО МЕРЕ КАСАНИЯ, а не сразу.** Переписать их все
одним махом значило бы тронуть семь экранов без единого повода со стороны продукта.

### РЕШЕНИЕ 2. Заголовок дашборда говорит «Всё готово» только при `allDone === true`

`apps/web/components/dashboard-stats.tsx`. Прежде заголовок читал проп `emailVerified` — то есть
поздравлял продавца, у которого подтверждена лишь почта, а остальные три шага из четырёх открыты.
Те самые три шага перечислялись открытыми в блоке непосредственно ПОД этим же заголовком.

**НЕИЗВЕСТНОЕ НЕ ЕСТЬ ГОТОВОЕ.** `readiness` равен `null` в двух случаях: пока запрос статистики в
полёте и когда ответ не прошёл `isSellerReadiness` (устаревший бандл во вкладке, неудавшийся
запрос). Оба дают «Добро пожаловать» — утверждение, верное при любом наборе открытых шагов.
Заявить готовность может только явное `allDone === true`. Обратный дефолт объявлял бы «Всё готово»
каждому на каждой загрузке страницы, до того как ответ пришёл, и ещё раз — всякому, у кого запрос
не удался.

Проп `emailVerified` остался в типе компонента, но больше не разбирается: контракт с вызывающим не
тронут, и изменение целиком помещается в один файл.

### ЧЕГО ЭТИМ НЕ РЕШЕНО

**В кабинете три разных красных, и два из них в палитре не объявлены вовсе.** Токен `text-error`
(`#dc2626`) — двенадцать вхождений; сырой `text-red-700` (`#b91c1c`) — тринадцать в восьми файлах,
включая все четыре сообщения об ошибке самой формы нового заказа; сырой `text-red-800` (`#991b1b`)
— девять, все в `shipments-page.tsx`. Ни `red-700`, ни `red-800` не описаны ни в `globals.css`, ни
в `tailwind.config.ts` — это дефолтная палитра Tailwind, а не палитра проекта.

Значит звёздочка `#dc2626` стоит на форме заказа рядом с ошибками `#b91c1c`. **Это принято
сознательно:** образец обязан стоять на палитре, а разнобой существует независимо от звёздочки.
**Миграция вынесена ОТДЕЛЬНЫМ СРЕЗОМ и механической быть не может.** Что ей мешает: под
`hover:bg-red-700` в палитре нет токена вовсе (`--error-hover` не существует, есть только
`--primary-hover`); `bg-red-50` не равен `bg-error-soft`, потому что `--error-soft` это `#fee2e2`,
то есть `red-100`, а не `red-50`; под `border-red-200` подмены нет — `border-error` это другой
элемент дизайна, а не оттенок; часть красных в `shipments-page.tsx` кодирует СТАТУС отправления
(`PROBLEM: "bg-red-100 text-red-800"`), а не ошибку интерфейса, и при этом `ui/status-badge.tsx`
красит свои статусы уже токеном — то есть две реализации статусных бейджей расходятся в цвете.
Контраст `#dc2626` на `bg-error-soft` НЕ ИЗМЕРЕН.

Отвергли: помечать не обязательные поля, а необязательные словом «необязательно» (помечать
пришлось бы большинство — необязательных полей на форме больше, чем обязательных); ставить
`aria-required` внутри `FieldLabel` (подпись не владеет состоянием контрола); делать `htmlFor`
необязательным ради краткости (именно необязательность и породила восемь висячих подписей); взять
для звёздочки `text-red-700` под цвет соседних ошибок (закрепило бы образцом для всего кабинета
цвет, которого в палитре нет); убрать проп `emailVerified` из `DashboardStats` совсем (это правка
вызывающего, к этому срезу не относится).

## 2026-09-08 (3) · Неактивная кнопка подключения научилась называть, чего ждёт

**ЗАКАЗАНО ЮЛИЕЙ 08.09.** Срез E2. Дефект был записан заранее — `docs/ROADMAP.md`, строка
«Неактивная кнопка на форме подключения не говорит, чего не хватает»: наблюдено 04.09.2026 живой
проверкой, логин и пароль заполнены, тип договора не выбран, кнопка серая и молчит. Причина
неполноты была известна коду и на экран не выводилась.

### РЕШЕНИЕ 1. Булева ВЫВЕДЕНА из описания и второй реализации не хранит

`isCarrierFormComplete` больше не решает ничего сама: её тело — проверка `kind === "ready"` на
результате `describeCarrierFormGap`. Правила (что считается отправляемым значением, чем отличаются
две ветки подключённости) переехали в новую функцию ЦЕЛИКОМ, а не были скопированы.

**Почему не две независимые реализации.** Кнопка и плашка обязаны отвечать одинаково. Две
реализации рано или поздно дают состояние «кнопка зовёт нажать, а плашка рядом перечисляет
недостающее» — и это хуже исходной серой молчащей кнопки, потому что молчание хотя бы не
противоречит само себе. **Расхождение сделано невозможным, а не проверяемым** — тем же приёмом, что
`carrier-connect-fields.ts`, который выводит дескрипторы формы из спецификации сервиса вместо того,
чтобы перечислять их заново.

**Сигнатура и тип возврата не изменились ни на символ**, поэтому двенадцать прежних тестов остались
зелёными БЕЗ единой правки: точечный прогон файла дал те же 30 до и сразу после вывода, а
`git diff` по нему в тот момент был пуст. Теперь они же стерегут вывод — ошибка новой функции на
границе «готово / не готово» роняет их. Отдельного теста «булева согласована с описанием»
сознательно НЕТ: при выводе он не может упасть, а страж, которого никто не видел падающим, ничего
не доказывает.

### РЕШЕНИЕ 2. Пять причин негодности сведены к ДВУМ смыслам

Поле непригодно в пяти случаях: ключа нет в значениях; значение не строка; строка пуста или из
одних пробелов; выбор вне собственного набора вариантов; у поля-выбора вовсе нет вариантов. Для
продавца смыслов **два**: поле пусто, либо выбранный вариант нельзя отправить.

Первые три уже сливаются в коде в ОДНУ точку возврата — различить их после неё нечем. Разделять их
на экране значило бы назвать деталь реализации вместо того, что продавец может исправить: разница
между отсутствующим ключом и строкой из пробелов не подсказывает ему ни одного действия.

**Ветка «подключён» не называет поля вовсе, и это не упрощение.** Там достаточно одного заполненного
поля, поэтому отказ означает «не годится НИ ОДНО», и назвать какое-то одно было бы неверно:
заполнение любого другого снимает отказ ровно так же. Фраза говорит о форме целиком —
«Чтобы сохранить, заполните хотя бы одно поле». Обратное следствие тоже верно и используется:
перечислимая нехватка возможна ТОЛЬКО у неподключённого перевозчика, поэтому её текст может смело
говорить «для подключения».

Ни в одной фразе нет числительного — перечисление идёт после двоеточия, поэтому одна формулировка
верна при любом количестве полей и ветки по числу, которая могла бы разойтись со списком, не
существует. Метки подставляются как хранятся, строчными: `capitalizeFieldLabel` здесь НЕ
вызывается, потому что метки и лежат строчными ради употребления в середине фразы, а заглавная
ставится на подписи поля при отрисовке. Два теста обходят каждое поле каждого перевозчика и
доказывают, что в текст не попадает ни внутренний ключ, ни что-либо введённое продавцом.

### РЕШЕНИЕ 3. Плашка ждёт касания карточки — переиспользование ЗАЩИТНОГО признака

Плашка появляется, только когда продавец тронул хотя бы одно поле этой карточки. Панель рисует всех
перевозчиков сразу, поэтому показ при открытии встретил бы человека янтарным блоком на каждой
неподключённой карточке до единого действия; а наблюдённый дефект — про другой момент, когда
заполнено два поля из трёх.

Признак касания — `interacted`, и он заведён НЕ для оформления. Он существует потому, что измерено:
Chrome автозаполнял в эти поля собственный сайтовый пароль продавца и при этом слал событие,
доходившее до состояния React, — кнопка отправки оживала, и нажатие отправило бы пароль от чужого
сайта перевозчику как токен API. `shouldAcceptFieldValue` решает по нему, что вообще попадает в
значения формы.

**Цена связи, и она принята сознательно.** Если браузер когда-нибудь станет фокусировать поле перед
заполнением, гейт примет такое значение — и момент показа плашки сдвинется вместе с гейтом. Это
косметика: свойство безопасности гейта не меняется ни при каком исходе. Отдельный признак `touched`
отвергнут — это была бы ТРЕТЬЯ параллельная запись той же формы, с тем же ключом
`[providerKey][fieldName]`, рядом со значениями и `interacted`, то есть второй экземпляр одного
факта, за расхождение которых этот репозиторий уже платил.

### ЧЕГО НЕ ИЗМЕРЕНО

**Роль `role="status"` выбрана рассуждением, а не проверкой.** Довод: текст плашки меняется на
каждое нажатие клавиши, и настойчивая область (`alert`) перебивала бы чтение на полуслове, тогда
как вежливая дожидается паузы. **Вспомогательными технологиями это не проверялось.** Записано так
же и в коде рядом с плашкой, чтобы следующий читатель не принял довод за проверенный факт.

**Контраст `bg-amber-50` / `text-amber-900` не измерен** — ни здесь, ни в форме нового заказа,
откуда цвет скопирован дословно. Цвет взят ради того, чтобы не заводить третий стиль, а не потому,
что его читаемость подтверждена.

### НАБЛЮДЕНИЕ ЖИВОЙ ПРОВЕРКИ 08.09.2026 — К ЭТОМУ СРЕЗУ НЕ ОТНОСИТСЯ

На заведомо мусорных учётных данных СДЭК форма подключения ответила «Перевозчик сейчас не отвечает.
Попробуйте позже.» — то есть отказ перевозчика показан как его молчание. Это подтверждение глазами
уже записанного дефекта (`docs/ROADMAP.md`, строка «Отказ перевозчика в доступе показывается как
«не отвечает»», наблюдено 04.09.2026). Здесь не чинится.

**Механизм по-прежнему НЕ УСТАНОВЛЕН, и записываю это, а не догадку.** Прочитано по коду сегодня:
эта фраза приходит только из `carrier_unavailable`, а туда ведёт единственный путь — вердикт
`unavailable`, который `verdictForOutcome` выдаёт на `server_error` (5xx после трёх попыток),
`config_error` и `transport_error`. Правило «400 с кодом, отличным от `validation_error`, считается
ACCEPTED» в коде действительно есть и принято сознательно (никто не может перечислить коды ошибок
перевозчика заранее; измерено 09.08: рабочие учётные данные отвечали 400 `pickups_not_configured`),
**но оно НЕ объясняет это наблюдение**: по нему подключение завершилось бы успехом и сообщением
«Перевозчик подключён.», а не «не отвечает». Значит отказ теряется где-то ещё. Кандидат, записанный
в ROADMAP, — известные перемежающиеся 500 песочницы, но это гипотеза, а не измерение, и проверяется
она замером, а не рассуждением.

Отвергли: менять `isCarrierFormComplete` или её тесты (двенадцать сравнений с булевыми литералами в
строгом режиме — тот самый закреплённый рубеж, который нельзя двигать заодно с правкой); держать
две независимые реализации и сторожить их согласующим тестом (проверяемое расхождение вместо
невозможного); различать на экране все пять причин (три из них неразличимы уже в коде, а разница
между ними продавцу нечего дать); называть поле в ветке «подключён» (там не годится ни одно, и любое
названное было бы произволом); показывать плашку сразу при открытии вкладки (упрёк на каждой
неподключённой карточке до единого действия); заводить отдельный `touched` (третья параллельная
запись той же формы); прятать плашку, когда рядом стоит красная плашка ответа перевозчика (правило
«один баннер, никогда два» с формы заказа сюда не переносится: там два баннера были соперничающими
ответами на ОДИН вопрос, здесь — ответы на разные вопросы о разных моментах, и продавцу, стирающему
поле для перенабора после отказа, нужны оба).

## 2026-09-11 · Три отказа, которые ещё могли пойти не так: гонка погашения, молчание подтверждённому, пустой список полей

**ЗАКАЗАНО ЮЛИЕЙ 11.09.** Срез E4 — починка по независимому ревью коммитов `1c5b547..3270fd6`. Ревью
нашло девять вещей; три починены здесь, шесть записаны ниже, в «ЧЕГО ЭТИМ НЕ ЗАКРЫТО», чтобы их
не открывали заново.

**Как доказано.** Путь верификации до этого среза не имел ни одного теста, а роут тестировать
нельзя (auth + Prisma + Next — CLAUDE.md, правило 1). Поэтому решение о том, куда отправить при
отказе, и само погашение вынесены в два модуля — `apps/web/lib/auth/verification-redirects.ts`
(чистый) и `apps/web/lib/auth/redeem-verification-token.ts` (клиент базы параметром). Сначала
вынос сделан с поведением роута БАЙТ В БАЙТ как было, и на нём оба дефекта закреплены тестами,
которые прошли — то есть воспроизвели дефект. Потом починка: оба закрепления упали. Потом
переворот. Файл — `tests/db/verify-email.db.test.mjs`, тринадцать тестов.

### РЕШЕНИЕ 1. Погашение токена условно, и условие проверяет база в том же UPDATE

**Окно, которое это закрывает.** Чтение искало строку по токену, запись приземлялась по `id`, и
между ними ничто не держало их вместе. Повторная отправка, пришедшая в этот зазор, перезаписывала
колонку новым токеном — после чего старый запрос подтверждал почту и **обнулял УЖЕ НОВЫЙ токен**,
убивая ссылку, которую человек только что запросил. Воспроизведено тестом: чтение по старому
токену → перезапись колонки, в точности как делает `issueVerificationToken` → запись. До починки
старый запрос возвращал «погашено», `emailVerified` становился `true`, а новый токен — `null`.

**Теперь** запись — `updateMany` с `where: { id, verificationToken: token }`, успех — `count === 1`.
Условие и запись — один `UPDATE … WHERE "id" = $1 AND "verificationToken" = $2`: Postgres проверяет
и пишет одним действием, и повторная отправка, пришедшая первой, просто оставляет WHERE без
совпадения. Ноль обновлённых строк означает «колонку перезаписали между чтением и записью», то есть
ссылка устарела, — и роут отвечает ровно как на устаревшую ссылку, через тот же выбор адреса.
Любая проверка «прочитать, потом записать» в коде приложения снова открыла бы то самое окно.

**Почему `updateMany`, а не `update`** (Prisma 6.19.3). `update` принимает такой же составной where,
но на «строки нет» бросает P2025 — и ожидаемый исход стал бы неотличим от настоящего сбоя базы:
`catch` роута отработал бы оба одинаково. `updateMany` возвращает счётчик, и ноль — обычный
результат для ветвления.

`id` стоит в where рядом с токеном, хотя токен уникален сам по себе: он называет строку, для
которой пишется эта запись, — ту, что нашло чтение, — и условие читается как то, что оно значит:
«этот пользователь, всё ещё держащий этот токен».

### РЕШЕНИЕ 2. Уже подтверждённая почта — отдельный исход отказа

**Найдено живой проверкой 11.09.2026.** Подтверждённый продавец, открывший старое письмо, уходил
на `/verify-email?stale=1`; страница видела подтверждённую почту и **молча** редиректила на
дашборд. Человек переходил по ссылке и не видел на экране ничего — это и выглядело как сломанная
ссылка. Комментарий у `refusedRedirect` утверждал, что подписанный попадёт на страницу с кнопкой;
для подтверждённого это было неправдой, и он переписан.

**Теперь исходов три:** нет сессии → `/verify-email/error`; почта не подтверждена →
`/verify-email?stale=1`; почта уже подтверждена → `/dashboard?verified=already`, где тост говорит
**«Email уже подтверждён — ссылка больше не нужна.»** Тост тот же, что показывает «Email подтверждён
✓» после обычного подтверждения: механизм не менялся, изменилось только то, что вместо одного
сравнения с `"true"` и одного вшитого текста теперь таблица.

**Значение `already` выдаётся и читается через один модуль.** Роут строит адрес из
`VERIFIED_ALREADY_PATH`, тост ищет значение в таблице того же модуля. Написанное на двух сторонах
порознь, при переименовании на одной оставило бы другую без совпадения — без ошибки, просто
молча: продавцу не сказали бы ничего, и никто бы этого не заметил. Это закреплено тестом, который
проверяет **наличие ключа**, а не разрешённое значение: у поиска есть умолчание `?? null`, и
сравнение текстов прошло бы и на переименованном ключе. Старое значение `true` по-прежнему
написано в роуте в строке успеха, как было до модуля; оно ни в одну из трёх находок не входит и
не трогалось.

**Тексты тоста лежат в `Map`, а не в объекте.** Значение приходит из адресной строки, и
`?verified=constructor` по объекту нашёл бы что-то через цепочку прототипов. Закреплено тестом
по общему списку `PROTOTYPE_KEYS`.

Ветку ограничения частоты это решение не трогает: она по-прежнему отвечает страницей ошибки и
сессию не читает — теперь уже потому, что любой из двух не-ошибочных адресов сказал бы
перебирающему, что его заметили.

### РЕШЕНИЕ 3. Пустой список полей перевозчика — отдельный вид `no_fields`

Проверяется **первым**, в обоих состояниях подключения. `every` по пустому списку истинно
вакуумно, поэтому неподключённая карточка без единого поля объявлялась готовой: кнопка
«Подключить» горела, а POST ушёл бы с пустыми учётными данными. Закреплено на неизменённом коде
разовым прогоном: `describeCarrierFormGap([], {}, false)` возвращал `{ kind: "ready" }`,
`isCarrierFormComplete([], {}, false)` — `true`.

**Почему не переиспользован `nothing_supplied`,** который подключённая ветка для пустого списка и
так возвращала. Его текст говорит «Чтобы сохранить…», и `carrierFormGapMessage` документирует его
как исход только для подключённой карточки. На неподключённой карточке с кнопкой «Подключить» это
была бы ложь, а документированное свойство перестало бы быть правдой.

**Сообщения у `no_fields` нет.** Это ошибка нашей конфигурации — запись перевозчика с пустым
списком учётных полей, — а не пробел продавца: называть нечего и чинить ему нечего. Кнопка остаётся
серой, плашка молчит, а не выдумывает указание. Через интерфейс это состояние сегодня недостижимо:
у обоих подключаемых перевозчиков поля есть.

Двенадцать тестов на `isCarrierFormComplete` остались без правок: пустого массива среди них не
было, и эталонный файл прогнан зелёным с пустым `git diff` **после** починки и **до** того, как в
него дописаны новые тесты.

### ЧЕГО ЭТИМ НЕ ЗАКРЫТО — шесть находок того же ревью, оставлены сознательно

1. **Клиент различает два ответа 429 по наличию цифры в тексте.** Кулдаун несёт число секунд,
   ограничение по адресу — нет, и кнопка отправки письма ветвится на `match(/(\d+)/)`. Смена текста
   ответа на стороне сервера молча переставит ветки.
2. **Устаревший бандл во вкладке возвращает прежнее поведение заголовка дашборда** — «Всё готово»
   по одной подтверждённой почте, до исправления от 08.09.
3. **Повторная отправка, отклонённая кулдауном, не пишет новый токен — и клиент об этом молчит.**
   После решения среза E3 кнопка при кулдауне показывает только обратный отсчёт; что письма не
   было и прежняя ссылка всё ещё рабочая, не сказано.
4. **Перезапись токена происходит ДО отправки письма.** Если отправка не удалась и роут ответил
   503, прежняя ссылка уже мертва, а новой у продавца нет.
5. **После десяти отказов по ограничению частоты подписанный снова попадает на тупиковую
   страницу.** Это обратная сторона решения 2: ветка лимита намеренно отвечает страницей ошибки
   и сессию не читает.
6. **`FieldLabel` не ставит `aria-required`** — это делает вызывающий на самом контроле, — и семь
   файлов кабинета с голым `required` по-прежнему ждут касания, как и решено 08.09.

Отвергли: проверять токен в коде приложения между чтением и записью (снова открывает окно);
`update` вместо `updateMany` (ожидаемый исход становится исключением); писать значение `already` на
двух сторонах порознь (переименование на одной молча обрывает связь); объект вместо `Map` для текстов
тоста (прототипные ключи из адресной строки); переиспользовать `nothing_supplied` для пустого списка
(его текст и документированное свойство относятся только к подключённой карточке); тестировать роут
напрямую (auth + Prisma + Next — решения вынесены, чтобы их можно было тестировать).

## 2026-09-11 (2) · Неудачная отправка письма больше не убивает ссылку, которая ещё работает — ценой серверной паузы

**ЗАКАЗАНО ЮЛИЕЙ 11.09.** Срез E5. Закрывает пункт 4 из «ЧЕГО ЭТИМ НЕ ЗАКРЫТО» записи 2026-09-11
выше: новый токен записывался ДО отправки письма, и неудавшаяся отправка оставляла продавца без
рабочей ссылки — ссылка в его почте уже перезаписана, новая существует только в нашей базе. Если
причина постоянная (например, не задана настройка), каждая следующая попытка повторяла то же самое.

**Как доказано.** `issueVerificationToken` тестом недостижим — он берёт синглтон `@oco/db` и алиас
`@/`, — поэтому всё, что он решает, вынесено в модули без импортов:
`apps/web/lib/auth/verification-send-outcome.ts` (откатывать ли, что записать, жива ли прежняя
ссылка, текст 503, строка лога), `apps/web/lib/auth/verification-page-copy.ts` (состояние письма и
тексты страницы), `apps/web/lib/auth/parse-resend-response.ts` (как кнопка читает ответ). Граница
исходов — `classifyVerificationSend` в `packages/core/lib/email.ts`. Сначала вынос сделан с
сегодняшним поведением: 36 тестов, из них 8 закреплений, все прошли на неизменённом коде. После
починки упали ровно 8 закреплений. Потом переворот; при перевороте дописаны три теста новой логики
(черта 499/500, имя переменной в логе без значения, текст про живую ссылку доходит до кнопки) — их
падающими никто не видел. Файл — `tests/verification-resend.test.mjs`, 39 тестов. `test:unit` 1832,
`test:db` 157, `typecheck` 0, сборка 0 с теми же семью предупреждениями, ни одного в файлах среза.

### РЕШЕНИЕ 1. Неудачная отправка откатывает токен — но только когда доставка исключена

`sendVerificationEmail` больше не бросает исключение, а возвращает один из трёх исходов. Шесть мест,
где отправка может кончиться неудачей, разведены по двум исходам, по три на каждый:

- **«Точно не ушло»** — нет переменных окружения (функция выходит до любого сетевого вызова);
  HTTP 4xx (провайдер отказал); 2xx с `status: "error"` в теле (провайдер сказал это сам).
- **«Не установлено»** — любой отказ `fetch` (код не отличает сбой до отправки запроса от сбоя
  после: это один и тот же брошенный `await fetch`); HTTP 5xx и прочие не-2xx (шлюз, не дождавшийся
  ответа, может стоять перед сервером, который письмо принял); 2xx с нечитаемым телом (HTTP-запрос
  принят).
- **«Ушло»** — 2xx, тело разобрано, статус не `error`.

Откат — только на «точно не ушло». На «не установлено» новый токен **остаётся**, как было до
среза: письмо могло дойти, и откат убил бы доставленную ссылку.

**Черта между 4xx и 5xx держится на семантике HTTP, а не на измеренном поведении Unisender.** Как он
отвечает на то и другое, никто не наблюдал. Это же сказано в комментарии у `classifyVerificationSend`.

**Откат условный:** `updateMany` с `where: { id, verificationToken: <наш токен> }`, данные — прежние
значения обоих столбцов; null возвращается как null, так что неудачное ПЕРВОЕ письмо возвращает
строку к виду до выдачи токена. Ноль обновлённых строк — не ошибка: после нас писал кто-то ещё,
например вторая вкладка с успешной отправкой, и его значение главнее. Тот же приём и по той же
причине, что в решении 1 записи 2026-09-11 выше.

**Прямая запись нового токена осталась безусловной.** Побеждает последняя отправка, как и раньше;
сделать эту запись условной значило бы превратить параллельную повторную отправку в отказ — другое
правило, и менять его не задача этого среза.

### РЕШЕНИЕ 2. ⚠ Размен, который ломает запись от 25.06.2026

ADR «Email verification (2026-06-25)» фиксирует: `resend cooldown 60s server-enforced`. После
отката это больше не так. Сервер считает паузу от момента выдачи токена, выводя его из
`verificationTokenExpiry`; после отката в столбце снова срок **прежнего** письма, и пауза держится
от него, а не от неудачной попытки. После неудачного первого письма столбец пуст, и серверной паузы
нет вовсе. Момент попытки хранить негде: в схеме только два столбца, `verificationToken` и
`verificationTokenExpiry`, и оба откатываются. Новая пауза 60 секунд после ответа 503 или 500
ставится только на клиенте (`parse-resend-response.ts`) и снимается перезагрузкой страницы. На
сервере остаётся лимит по IP.

Размен принят сознательно: рабочая ссылка продавца дороже паузы.

⚠ **Цель паузы нигде не записана.** У константы `VERIFICATION_RESEND_COOLDOWN_MS`
(`apps/web/lib/auth/verification.ts`) комментария нет; ADR от 25.06 называет только число. Защита
от спама в этом файле записана для лимита по IP и для порядка «лимит → авторизация» (ADR `withAuth`
и ADR send-verification от 05.07), но не для самой паузы. **Если цель паузы была в том, чтобы беречь
квоту провайдера, к этому решению надо вернуться.**

### РЕШЕНИЕ 3. Что говорится продавцу и что — оператору

**503 выбирает между двумя текстами** по трём условиям сразу: откат прошёл (`count === 1`), прежний
токен был не null, его срок не истёк. Если выполнены все три — «Не удалось отправить новое письмо.
Ссылка из предыдущего письма по-прежнему действует.» Иначе — прежний общий текст «Не удалось
отправить письмо. Попробуйте позже.», без изменений. Ответ 500 (сбой внутри роута) — тот же общий
текст.

**Страница `/verify-email`**, когда почта не подтверждена и `verificationTokenExpiry` равен null,
говорит «Письмо не отправлено» вместо «Проверьте почту»: строка «Не удалось отправить письмо на
{email}. Отправьте его ещё раз.», фразы про 24 часа нет, кнопка — «Отправить письмо». Условие
читается из базы, нового параметра в адресе нет. Подтверждённый продавец до этой проверки не
доходит: страница уводит его на дашборд раньше.

**Причина отказа идёт в серверный лог** одной строкой в форме `serverLog` из
`connect-result-response.ts`: `[auth/verification-email] <причина>; <что стало с токеном>`. Причина
называет имя переменной, HTTP-статус или код ошибки — никогда значение, тело ответа, текст
исключения или адрес. Для отказа `fetch` в лог идёт `cause.code`, если он похож на код, иначе имя
ошибки. Строку пишут оба роута — повторной отправки и регистрации. Продавцу причина не показывается.

### ЧЕГО ЭТИМ НЕ ЗАКРЫТО

1. **При исходе «не установлено» ответ говорит «Попробуйте позже»,** хотя письмо могло дойти.
2. **Условный откат тестами не покрыт** — `updateMany` с токеном в where не прогонялся ни одним
   тестом.
3. **Тест на сам `issueVerificationToken` невозможен,** пока модуль тянет синглтон `@oco/db` и алиас
   `@/`. Передача клиента параметром — отдельный срез.
4. **Учётки, созданные до появления проверки почты, увидят «Письмо не отправлено»** — у них
   `verificationTokenExpiry` пуст. Сколько их, не измерено.
5. Пункт 1 записи 2026-09-11 (кнопка различает два ответа 429 по цифре в тексте) перенесён в
   `parse-resend-response.ts` без изменения логики и остаётся открытым.

Отвергли: откатывать на любой неудаче (на «не установлено» убил бы доставленную ссылку); булев
исход отправки (сливает «не ушло» и «не установлено»); безусловный откат (стёр бы токен успешной
отправки из второй вкладки); условную прямую запись (меняет правило «побеждает последняя
отправка»); показывать причину продавцу (ему нужно знать, что работает, а не имя переменной).

## 2026-09-11 (3) · Выпуск токена стал достижим для теста, и восемь из девяти случаев показаны падающими — через колонку, а не через ответ

**ЗАКАЗАНО ЮЛИЕЙ 11.09.** Срез E6. `issueVerificationToken` (`apps/web/lib/auth/verification.ts`)
брал синглтон `@oco/db` через алиас `@/lib/db`, и тест базы не мог его вызвать: алиас вне Next не
разрешается, а синглтон смотрит в базу разработчика.

### РЕШЕНИЕ 1. Клиент базы — первым аргументом, как у ограничителя частоты

Форма и шапка с объяснением взяты у `apps/web/lib/auth/rate-limit.ts` после коммита `1e970d9` и
перенесены дословно. Импорт соседнего модуля сделан относительным (`./verification-send-outcome`).
Вызовов, которые передают синглтон, два в двух файлах: `apps/web/app/api/auth/register/route.ts:76`
и `apps/web/app/api/auth/send-verification/route.ts:53`. Функции паузы (`isResendCooldownActive`,
`resendCooldownRemainingSec`, `verificationTokenIssuedAt`) не тронуты: в базу они не ходят, и их
вызовы в `verify-email/page.tsx` и `cabinet-shell.tsx` остались как были.

### РЕШЕНИЕ 2. Отправитель писем параметром НЕ стал

`packages/core/lib/email.ts` не имеет ни одного импорта и ходит к провайдеру через глобальный
`fetch`, поэтому тест подменяет `fetch`, а не отправителя — так же, как восемнадцать тестов
перевозчиков (`installFetchMock`, локальная функция в каждом из них). Через подменённый `fetch`
достижимы все шесть путей неудачи. Переменные почты подменяются формой `withEnv` из
`tests/yandex-get-order-info.test.mjs`: прежнее значение запоминается, ставится подставное или
удаляется, в `finally` возвращается прежнее. **`.env` не читается и не меняется.** Подставные
значения стоят в каждом случае, так что настоящий ключ, окажись он в процессе, не попадёт ни в
подменённый `fetch`, ни в текст упавшей проверки. `packages/core` не тронут. Импорт `@oco/core` из
теста под `tsx` разрешился — измерено первым прогоном, до него это было не проверено.

### РЕШЕНИЕ 3. Девять случаев на границе «откатывать / не откатывать»

Файл `tests/db/issue-verification-token.db.test.mjs`, набор `test:db`: 1 — удачная отправка пишет
новый токен и срок; 2 — нет переменных почты, откат; 3 — HTTP 4xx, откат; 4 — 2xx со
`status: "error"`, откат; 5 — HTTP 5xx, не откат; 6 — 2xx с нечитаемым телом, не откат; 7 — отказ
`fetch`, не откат; 8 — первое письмо, обе колонки снова null; 9 — чужая запись между нашей записью и
откатом выживает.

**Показаны падающими восемь из девяти, тремя поломками** в `verification.ts`. Каждая ставилась на
копию файла, и после каждой файл возвращался из копии со сверкой хеша SHA256 и зелёным прогоном:

- (а) откат без условия по токену (`where: { id }`) — роняет 9;
- (б) откат всегда (`if (true)` вместо выбора по исходу) — роняет 5, 6, 7;
- (в) откат никогда (`if (false)`) — роняет 2, 3, 4, 8 и 9.

**Случай 1 не падал ни на одной.** Поломка, которая его уронит, — снять безусловную запись токена, —
по разбору кода роняет заодно шесть других (2–7): откат ищет строку по токену, который так и не был
записан. Прогоном это не проверялось. Такой охват сделал бы доказательство хуже, а не лучше, и
поломка не применялась.

### РЕШЕНИЕ 4. Порядок утверждений внутри случая — решение, а не оформление

Сначала проверяется **КОЛОНКА**, потом ответ функции (`priorLinkAlive`, строка лога). До
перестановки шесть падений из восьми приходились на первую же проверку ответа, и состояние базы на
поломке не проверялось вовсе. После — восемь из девяти через колонку. Колонка — это то, что
останется у продавца; ответ функции — то, что мы про это говорим, и второе без первого ничего не
стоит. Порядок изменён в случаях 2–7, сами утверждения не менялись; причина записана в шапке файла.

**ОГРАНИЧЕНИЕ.** Случай 9 не различает состоянием «условный откат уступил» и «отката не было».
Обработчик подменённого `fetch` перезаписывает колонку ДО отката, поэтому в обоих исходах в базе
чужая запись целиком, со сроком; `updateMany` без совпадения не меняет ни одной строки. Различает
только текст лога (`rollback skipped`) — поэтому на поломке (в) случай 9 падает на ответе функции,
а не на колонке. Что откат вообще условный, доказывает поломка (а): там упала колонка.

### ЧТО ЭТИМ ЗАКРЫТО из записи 2026-09-11 (2)

Пункты 2 и 3 её раздела «ЧЕГО ЭТИМ НЕ ЗАКРЫТО» (см. запись `2026-09-11 (2)` выше): условный откат —
`updateMany` с токеном в where — теперь покрыт случаем 9 и поломкой (а); тест на сам
`issueVerificationToken` существует — девять случаев выше. Пункты 1, 4 и 5 той записи остаются
открытыми. В ROADMAP в том же срезе утверждение «`issueVerificationToken` … не вызывает ни один
тест» помечено как неверное с 11.09.2026, прежний текст сохранён цитатой.

## 2026-09-11 (4) · Кнопка повторной отправки читает код отказа, а не ищет цифру в тексте

**ЗАКАЗАНО ЮЛИЕЙ 11.09.** Срез E7. Находка независимого ревью от 11.09 — пункт 1 раздела «ЧЕГО ЭТИМ
НЕ ЗАКРЫТО» записи `2026-09-11`. Роут `/api/auth/send-verification` отвечает 429 в двух разных
случаях — пауза на аккаунт и лимит по IP, — и кнопка различала их по тому, есть ли в тексте цифра:
`match(/(\d+)/)`, после среза E5 — в `apps/web/lib/auth/parse-resend-response.ts:35`.

### РЕШЕНИЕ. Оба ответа 429 несут машинный код рядом с текстом

Клиент решает по коду, а не по наличию цифры в тексте. Форма взята у `carrier-picker/recommend` —
поле `reason` с кодом в snake_case (`apps/web/lib/carrier-picker/recommend.ts:30,85`; коды
`no_active_carrier`, `weight_required` — `packages/core/src/carrier-picker/rank.ts:246,338`). Коды
этого роута: `resend_cooldown` — пауза на аккаунт, `ip_rate_limited` — лимит по IP. Они живут одной
константой `RESEND_REFUSAL_REASON` в `apps/web/lib/auth/parse-resend-response.ts`, которую роут
импортирует у парсера: выдаёт и читает одно значение, а не два написания.

Секунды паузы отданы отдельным полем `retryAfterSec`. Заголовок `Retry-After` не тронут — он
стандартный и полезен вне нашего клиента, но читать секунды из заголовка, а признак из тела значило
бы завести два источника. Кнопка берёт секунды только из тела и только целым положительным числом;
всё иное секундами не считается.

Тексты ответов не изменены ни на символ: именно они и были тем, что случайно различало два отказа.
Сохранены они ещё и потому, что вкладка со старым бандлом, открытая до выкладки, по-прежнему
разбирает текст.

### ОТСУТСТВИЕ КОДА ЧИТАЕТСЯ КАК «ГОВОРИТЬ ВСЛУХ»

Молчаливый отсчёт выглядит в точности как после удачной отправки, и продавец решит, что письмо
ушло. Поэтому ответ без кода, незнакомый код и пауза без годных секунд — все дают паузу 60 секунд и
текст, а не молчание. Отсчёт без красной строки получает только `resend_cooldown` с годными
секундами.

**Поправка по факту к формулировке задания.** Среди этих случаев задание называло «старый бандл во
вкладке». Это не так: старый бандл — это старый КЛИЕНТ, до новой ветки он не доходит и по-прежнему
ищет цифру в тексте; сервер тексты не менял, поэтому такая вкладка ведёт себя как до среза. Ветку
«кода нет» проходит новый клиент, получивший ответ без кода, — от сервера старее этого поля или из
кэша.

⚠ **ЧЕГО НЕ ДОКАЗАНО.** Что роут действительно кладёт новые поля в ответ: роуты здесь не
тестируются (CLAUDE.md, правило 1), держат только общая константа и typecheck. Проверяется живой
проверкой — нажать кнопку во время паузы: должен идти только отсчёт, без красной строки. И: код
`ip_rate_limited` для кнопки сегодня ничего не меняет — он идёт тем же путём «вслух», что и
незнакомый код, и тестом нельзя показать, что парсер его «узнаёт». Отдаётся для остальных
читателей ответа.

**Тесты** — `tests/verification-resend.test.mjs`, 47 случаев (было 39). Тест «429 with a number»
закреплён зелёным на неизменённом коде, упал сразу после правки парсера и перевёрнут: тело с
текстом паузы, но без кода теперь даёт паузу и текст. Тест «429 without a number» НЕ упал и не
перевёрнут: ответ «кода нет — говорим вслух» совпадает с тем, что лимит по IP получал всегда, и
теперь он закрепляет путь «поля нет». Восемь новых: оба кода; секунды из поля, а не из текста;
незнакомый код; пауза без годных секунд; код без текста — для паузы и для лимита по IP;
различимость кодов.

### ЧТО ЭТИМ ЗАКРЫТО

Пункт 1 раздела «ЧЕГО ЭТИМ НЕ ЗАКРЫТО» в записи `2026-09-11` выше («Клиент различает два ответа 429
по наличию цифры в тексте») и пункт 5 того же раздела в записи `2026-09-11 (2)` выше — тот же
дефект, перенесённый в `parse-resend-response.ts` без изменения логики. Тот же класс дефекта
остаётся на форме нового заказа: строка в `docs/ROADMAP.md`, группа 1 сводного списка.

## 2026-09-11 (5) · Слова APIShip больше не доходят до продавца — ни через ответ, ни через лог

**ЗАКАЗАНО ЮЛИЕЙ 11.09.** Срез E8. Прямой запрет CLAUDE.md: «Never put provider response bodies into
error messages». Найдено при STEP 0 среза E8.

### РЕШЕНИЕ. Продавцу — собственный текст роута, оператору — статус и код

Тело ответа APIShip доходило до продавца в **ЧЕТЫРЁХ** роутах, не в двух:
`apps/web/app/api/shipments/intervals/route.ts`, `create/route.ts`, `calculate/route.ts`,
`points/route.ts`. Во всех четырёх `ApishipError.message` — это `body.description ?? body.message`
из ответа провайдера (`packages/integrations/apiship/src/client.ts:295–296`, `auth.ts:50–51`); в
`create` оно приходит через `apps/web/lib/shipments/create-shipment.ts:230–231`, который пробрасывает
ошибку как есть.

**Закрепление до починки воспроизвело утечку — на подставной строке.** Тест положил в `message`
выдуманную строку в форме эха отправленных данных — ФИО, телефон и адрес получателя, — и во всех
четырёх роутах она дошла до тела ответа продавцу. **Поправка по факту к формулировке задания:** это
доказывает, что продавцу уходит ЛЮБОЙ текст из `message`. Что APIShip действительно возвращает в
`description` данные получателя, **не измерено** — боевого ответа с ними не наблюдали.

Решение вынесено в чистый модуль `apps/web/lib/apiship-failure-response.ts`, по форме
`apps/web/lib/carriers/connect-result-response.ts`. Продавцу — собственный текст роута, тот, что
раньше стоял после `||`; новых формулировок нет, ответ по-прежнему 502. В серверный лог — область,
HTTP-статус и код, если он похож на код по той же проверке, что в `packages/core/lib/email.ts`
(`/^[A-Z][A-Z0-9_]{1,63}$/`): код тоже берётся из тела ответа (`client.ts:298`, `body.code`). В
`intervals` новая строка заменила прежний лог, который писал `code` как есть.

⚠ **MESSAGE НЕ ПИШЕТСЯ ДАЖЕ В ЛОГ, и это не осторожность.** У одного из путей создания
`ApishipError` в `message` лежит `JSON.stringify` сырого ответа, и HTTP-статуса там нет:
`packages/integrations/apiship/src/client.ts:508–510`, ветка `/orders/status`, «нераспознанная форма
ответа». По полям ошибки такой случай от безопасного не отличить.

**Тесты** — `tests/apiship-failure-response.test.mjs`, 12 случаев. Закрепление «PIN · a provider
message → today: it reaches the seller as is» прошло на выносе без изменения поведения, упало сразу
после починки — единственным из шести — и перевёрнуто. Новые: область и статус в логе; ни одного
фрагмента сообщения в логе; похожий на код код пишется, иной — нет; отсутствие статуса названо; сырой
ответ без статуса не доходит ни до продавца, ни до лога.

### ЧТО ЭТИМ ПОТЕРЯНО

Вместе с чужими сообщениями перестали доходить наши собственные: «Компания не найдена», «APIShip не
настроен: задайте APISHIP_BASE_URL в .env» (`apps/web/lib/apiship-client-for-company.ts:54, 59, 78,
89`). Тип тот же — `ApishipError`, — и по типу их не отличить. Оператор увидит «APIShip call failed
without an HTTP status». **Не чинится сейчас:** все четыре роута из формы нового заказа недостижимы,
а отсутствие настройки роуты обычно ловят раньше — ответом 400 от `canUseApiship`. Строка в
`docs/ROADMAP.md`, группа 1 сводного списка.

### НЕ ИЗМЕРЕНО

Формат кодов APIShip: если они строчные или числовые, в логе будут помечены как `withheld`. И: что
роуты действительно печатают `serverLog` и отдают `mapped.body`, держится на typecheck и диффе —
роуты здесь не тестируются (CLAUDE.md, правило 1).
