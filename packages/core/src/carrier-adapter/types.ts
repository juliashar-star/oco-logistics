...
export type CarrierOrderPlace = {
  /**
   * Номера места здесь НЕТ намеренно. Нумерация принадлежит адаптеру: она про
   * накладную перевозчика, а не про то, что объявил продавец. Порядок мест —
   * это порядок элементов массива.
   */
  weightG: number;
  lengthCm?: number;
  widthCm?: number;
