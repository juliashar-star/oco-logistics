...
test("formatOfferInterval: different Moscow days → dayFrom HH:MM — dayTo HH:MM", () => {
  ...
  assert.equal(
    ...
    "27 июля 13:00 — 28 июля 18:00",
  );
});

test("formatOfferInterval: absolute date, with no weekday and no relative label", () => {
  ...
  assert.equal(label, "29 июля, 09:00–18:00");
});
...
test("formatOfferInterval: Moscow zone — UTC evening is next calendar day in Moscow", () => {
  ...
  // Interval still on Moscow 28 Jul renders as 28 July, never as 29
  ...
  assert.equal(onPrevMoscowDay.includes("29 июля"), false);

  // Interval on Moscow 29 Jul renders as 29 July
  assert.equal(
    formatOfferInterval(
...
test("formatOfferDeliveryDays: only one usable → single-day label", () => {
  const now = new Date("2026-07-27T06:00:00.000Z"); // Mon 27 Jul Moscow
  assert.equal(formatOfferDeliveryDays("2026-07-27", "", now), "27 июля");
  assert.equal(formatOfferDeliveryDays("", "2026-07-28", now), "28 июля");
  assert.equal(formatOfferDeliveryDays("2026-07-29", "", now), "29 июля");
});
