...
### СКРИПТ БЕЗ BOM ЧИТАЕТСЯ КАК ANSI — ещё один молчаливый ноль
Измерено 08.09.2026. PowerShell 5.1 читает `.ps1` без BOM в кодировке ANSI, поэтому
кириллический литерал внутри скрипта калечится, а скрипт продолжает работать и
возвращает пустой результат. Наблюдено: сравнение тарифных книг искало листы по имени
«Оплат» и дало НОЛЬ листов оплаты во всех трёх книгах — при том, что в двух из них
такие листы есть. Ошибки не было, был ложный ноль.
**Кириллицу в исполняемый скрипт не класть.** Либо собирать строку из кодов символов
и держать исходник чисто в ASCII, либо писать файл с BOM. Это тот же класс, что
`grep -i` по кириллице выше: инструмент вернул пусто, и пусто выглядело как факт.
**Ноль от скрипта, который ищет по русскому слову, недействителен, пока не показано,
что литерал дожил до сравнения** — например, найдя тем же прогоном заведомо
существующее вхождение.
